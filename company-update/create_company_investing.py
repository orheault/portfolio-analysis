...
class InvestingCompanyCreater:
    ...
    def execute(self):
        securities = self.session.query(Security).filter(Security.company_id==None).filter(Security.security_type_id==SecurityType.SECURITY_TYPE_COMMON_STOCK).filter(Security.is_active==True).all()
        ...
        for security in tqdm(securities):
            ...
            try:
                time.sleep(2)
                invest_data = investpy.search_quotes(text='GOCO', products=['stocks'], countries=countries, n_results=1)
            except:
                pass
            ...
            if invest_data == None:
                self.disable_security(security)
                continue
            elif len(invest_data) < 1:
                self.disable_security(security)
                continue

            name = invest_data[0].name
